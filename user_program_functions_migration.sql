-- =====================================================
-- USER PROGRAM FUNCTIONS MIGRATION
-- Extends admin functions with identical structure for users
-- =====================================================

-- First, enhance the programs table to support user-created programs
...
ALTER TABLE programs ADD COLUMN IF NOT EXISTS visibility TEXT CHECK (visibility IN ('private', 'public', 'shared')) DEFAULT 'private';

-- Create user_programs table for program ownership and sharing
CREATE TABLE IF NOT EXISTS user_programs (
  ...
  -- Progress tracking (same as admin structure)
...
-- RLS Policies for user_programs (consistent with admin pattern)
...
-- RLS Policies for program_shares 
...
TO authenticated
USING (
  (program_type = 'user' AND created_by = auth.uid()) OR
...
FOR ALL
TO authenticated
USING (program_type = 'user' AND created_by = auth.uid())
WITH CHECK (program_type = 'user' AND created_by = auth.uid());

-- =====================================================
-- USER PROGRAM FUNCTIONS (IDENTICAL STRUCTURE TO ADMIN)
-- =====================================================

-- Function to create programs as user (mirrors admin structure exactly)
...
-- Function to update programs as user (identical structure to admin)
...
-- Function to delete programs as user (identical structure to admin)
...
RETURNS TABLE(share_id UUID, shared_with_user_id UUID, program_name TEXT)
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
...
BEGIN
  ...
  -- Create share record (update shared_count in programs)
  UPDATE programs 
  ...
  RETURNING program_shares.id, target_user_id, program_name;
END;
$$;
...
-- Function to add program to user's collection (from Explore)
CREATE OR REPLACE FUNCTION add_program_to_user_collection(
  program_id UUID
)
RETURNS TABLE(user_program_id UUID, program_name TEXT)
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
...
BEGIN
  ...
  ON CONFLICT (user_id, program_id) DO NOTHING
  RETURNING user_programs.id, program_name;
END;
$$;

-- Function to get user's programs (consistent with admin queries)
...
BEGIN
  ...
  SELECT 
    ...
    p.is_shareable, p.visibility, up.access_type, up.completion_percentage,
    p.created_at, p.updated_at, p.created_by
  FROM programs p
  LEFT JOIN user_programs up ON p.id = up.program_id AND up.user_id = auth.uid()
  WHERE 
    (p.created_by = auth.uid() AND p.program_type = 'user') OR  -- User's created programs
    (up.user_id = auth.uid())  -- User's added/shared programs
  ORDER BY 
    CASE WHEN p.program_type = 'user' AND p.created_by = auth.uid() THEN 1 ELSE 2 END,
    p.updated_at DESC;
END;
$$;

-- =====================================================
-- ROUTINE FUNCTIONS FOR USERS (SAME STRUCTURE)
-- =====================================================

-- Create routine as user (identical to admin pattern)
...
-- Update routine as user (identical to admin pattern)
...
-- Delete routine as user (identical to admin pattern)
